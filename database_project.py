...
connect_str = "dbname='project' user='postgres' host='localhost' " + \
              "password='azim'"
...
def query():
	...
	inner join coupon on coupon.id = owns_coupon.coupon_id inner join restaurant on coupon.restaurant_id=restaurant.id where person.name= 'azim' and coupon.valid > now() order by coupon.discount desc""")
	...
	has_cuisines.restaurant_id = restaurant.id where person.name='azim'""")
    result2 = cursor.fetchall()
    cursor.execute("""select restaurant.name,restaurant.address,sum(review.ambiance_score+review.price_score+review.foodquality_score+review.service_score+review.experience_score)/5 as avg_score from 
    ...
    group by restaurant.id order by price_rating desc""")
